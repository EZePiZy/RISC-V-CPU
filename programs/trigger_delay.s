# Trigger is set by bit 1 of register x31 (t6)
# Bit 0 of x31 is set to 1 everytime a second passes 

trig_loop:                  # loop while trigger is not 1
    andi t0, t6, 2          # and with second bit (trigger)
    addi t0, t0, -2         # check if its zero
...
wait_second:                # subroutine that does nothing while 1 second pulse is not high
    andi t1, t6, 1          # check first bit (1 second pulse)
    addi t1, t1, -1         # subtract to check if it's one
